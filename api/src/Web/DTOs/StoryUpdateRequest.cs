...
// PUT is a full replacement, so the shape matches StoryCreateRequest plus CriteriaDone,
// which only ever changes through an update. It lives in its own record so both can
// diverge later without breaking the other.
public record StoryUpdateRequest
{
    ...
    [Range(0, int.MaxValue, ErrorMessage = "Los criterios cumplidos no pueden ser negativos.")]
    public int CriteriaDone { get; init; }
